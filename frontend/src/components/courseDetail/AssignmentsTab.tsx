 ...
 */
export function AssignmentsTab({ courseSlug }: AssignmentsTabProps) {
    ...
    <section>
      ...
      {empty ? (
        ...
        <div className="space-y-2">
          {sorted.map((a) => (
            <AssignmentRow key={a.slug} courseSlug={courseSlug} assignment={a} />
          ))}
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      {isExam ? (
        <span
          className={
            "flex-shrink-0 rounded-md border border-accent/40 bg-accent/10 px-2.5 py-1 font-num text-xs text-fg " +
            (done ? "line-through" : "")
          }
          title="Exams can't be rescheduled by dragging — edit the assignment kind to move the date."
        >
          {formatDate(assignment.due_date)}
        </span>
      ) : (
        <label className="relative flex-shrink-0">
          <span className="sr-only">Due date for {assignment.name}</span>
          <span
            className={
              "pointer-events-none block w-[7.5rem] rounded-md border border-border bg-surface px-2.5 py-1 text-right font-num text-xs text-fg " +
              (done ? "line-through" : "")
            }
          >
            {formatDate(assignment.due_date)}
          ...
          <input
            ...
            aria-label={`Reschedule ${assignment.name}`}
          />
        </label>
      )}
      <button
